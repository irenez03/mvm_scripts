...
if __name__ == "__main__":
    ...
    sample_teams_data = {
        ...
        "famous": {"leo_s", "kaiki", "hairuo","elena","joey"},   # if 'leo_s' is here, put 'leo_s'
    ...
    # 1) Count all valid setlists
    total = solver.count_valid_setlists()
    print(f"\nTotal valid setlists found: {total}")



 

# 2) Find setlists with fixed start, end, AND MULTIPLE fixed middle teams
    try:
        ...
        target_team_1 = "xoxz"
        target_position_index_1 = 8  # 9th position
        
        # Second positional constraint
        target_team_2 = "bad_villain" 
        target_position_index_2 = 7  # 5th position

        target_team_3 = "siren"
        target_position_index_3 = 11  # 5th position

        target_team_4 = "drip"
        target_position_index_4 = 14  # 5th position
        # -----------------------------------------
        
        print(
            f"\n--- Finding setlists starting with '{start}', ending with '{end}', "
            f"with '{target_team_1}' in position {target_position_index_1 + 1} "
            f"AND '{target_team_2}' in position {target_position_index_2 + 1} "
            f"AND '{target_team_3}' in position {target_position_index_3 + 1}"
            f"AND '{target_team_4}' in position {target_position_index_4 + 1} ---"
        )
        
        found_count = 0
        limit = 10 # Let's find up to 5 examples
        ...
        for setlist in generator:
            # Check the first constraint AND the second constraint
            if (len(setlist) > target_position_index_1 and setlist[target_position_index_1] == target_team_1) and \
               (len(setlist) > target_position_index_2 and setlist[target_position_index_2] == target_team_2) and \
                (len(setlist) > target_position_index_3 and setlist[target_position_index_3] == target_team_3) and \
                (len(setlist) > target_position_index_4 and setlist[target_position_index_4] == target_team_4):
